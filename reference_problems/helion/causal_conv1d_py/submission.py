...
# Search for the secret NVIDIA tuning files
acf_files = glob.glob("/opt/booster_pack/causal_conv_*.acf")

# Tell the autotuner to search over multiple block sizes, warps, and stages
# We will use this when we run `eval.py benchmark`
TUNE_CONFIG = helion.Config(
    block_sizes=[1, 32, 64, 128], num_warps=[1, 2, 4, 8], num_stages=[1, 2, 3, 4]
)
if acf_files:
    TUNE_CONFIG.advanced_controls_file = acf_files

# --- HARDCODED LEADERBOARD CONFIGS ---
# These are just placeholders to pass the correctness checks.
# You MUST replace the benchmark configs with the tuned outputs later.
SHAPE_CONFIGS: dict[tuple, helion.Config] = {
    ...
    # Benchmark shapes (MUST TUNE THESE LATER)
...
def _make_kernel(config: helion.Config):
    # Use autotune_search_acf in the decorator to search ACFs when autotuning locally
    ...
    def kernel(
        ...
        # Tile across Batch, Depth, and Sequence.
        for rb, rd, rs in hl.tile([B, D, N], block_size=[1, None, None]):
            bi = rb.begin

            # 1. Initialize our single, clean accumulator
            acc = hl.zeros([rd, rs], dtype=torch.float32)

            # 2. Perform the 1D convolution
            for j in range(W):
                # Load the filter weight for this step
                c = w[rd, j].to(torch.float32)

                # Load the corresponding input sequence step
                x = hl.load(x_pad, [bi, rd, rs.index + j]).to(torch.float32)

                # Accumulate the result
                acc = acc + x * c[:, None]

            # 3. Add the bias offset
